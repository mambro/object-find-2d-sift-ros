...
void SiftGPUWrapper::detect(const cv::Mat& image, cv::vector<cv::KeyPoint>& keypoints, std::vector<float>& descriptors, const cv::Mat 	& mask) const {
    if (error) {
        keypoints.clear();
    }
    ...
    for (int i = 0; i < num_features; ++i) {
        cv::KeyPoint key(keys[i].x, keys[i].y, 6.0 * keys[i].s, keys[i].o); // 6 x scale is the conversion to pixels, according to changchang wu (the author of siftgpu)
...
void SiftGPUWrapper::cvMatToSiftGPU(const cv::Mat& image, unsigned char* siftImage) const {
    cv::Mat tmp;
